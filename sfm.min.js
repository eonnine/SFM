...
"mouseenter",this.preventEvent.bind(this)).addCustomEvent(b,"change",this.addNewFile.bind(this))};d.prototype.openInputFile=function(a){this.preventEvent(a);this.getElement(this.elementId.fileInputElement).click()};d.prototype.addNewFile=function(a){this.preventEvent(a);for(var b=a.target.files||a.dataTransfer.files,c=0,e;e=b[c];c++){if(!this.addFileStopValidator(e))return;var d=this.makeItemKey(e.name);e=this.createNewFile(e,d);this.removeDefaultHTML();this.addFileToFileMap(e,!0);this.addItem(e)}a.target.value=
""};d.prototype.addFileStopValidator=function(a){var b=a.name;b=b.substring(b.lastIndexOf(".")+1,b.length);var c=this.getConfig("file","file_extension"),e=this.getConfig("file","file_extension_except"),d=this.getConfig("file","file_count");if(a.size>this.getConfig("file","file_size"))return alert(this.getConfig("message","file_size_max_overflow")),!1;if(0!==d&&d<this.fileMap.size())return alert(this.getConfig("message","file_count_over")),!1;for(a=0;d=c[a];a++)if(b!==d)return alert(this.getConfig("message",
"file_extension")),!1;for(a=0;c=e[a];a++)if(b===c)return alert(this.getConfig("message","file_extension_except")),!1;return!0};d.prototype.createNewFile=function(a,b){try{var c=new Blob([a],{type:a.type})}catch(e){c=a}c.name=b;c.lastModified=a.lastModified||0;c.lastModifiedDate=a.lastModifiedDate||0;return c};d.prototype.makeItemKey=function(a){for(var b=a,c=1,e;this.fileMap.isContainsKey(b);)c++,e=a.substring(a.lastIndexOf("."),a.length),b=a.substring(0,a.lastIndexOf(".")),b=b+" ("+c+")"+e;return b};
d.prototype.addFiles=function(a){if(!Array.isArray(a))throw new SyntaxError("[SFM] argument type of `addFiles` must be Array");for(var b=0,c;c=a[b];b++)f.call(c,"name")?(this.removeDefaultHTML(),this.addFileToFileMap(c,!1),this.addItem(c)):console.error('[SFM] not exists attribute "name" in file object',c)};d.prototype.addFileToFileMap=function(a,b){a.isNewFile=b;this.fileMap.put(a.name,a)};d.prototype.removeDefaultHTML=function(){0==this.fileMap.length&&(this.getElement(this.elementId.fileArea).innerHTML=
"")};d.prototype.insertDefaultHTML=function(){0==this.fileMap.length&&(this.getElement(this.elementId.fileArea).innerHTML=this.fileAreaInnerHTML)};d.prototype.addItem=function(a){var b=this.addSeparator(this.elementId.fileRemove,a.name),c=this.addSeparator(this.elementId.fileDownload,a.name),e=this.getElement(this.elementId.fileArea),d=this.getConfig("key"),f=this.elementId.itemKeyArea,g="",h=this.addSeparator(this.elementId.item,a.name),k=this.getConfig("item")(a,b,c);Object.keys(d).forEach(function(b,
c){0===c&&(g="<span "+f+">");g+='<input type="hidden" id="'+b+'" value="'+(a[b]?a[b]:"")+'"/>';c==d.length-1&&(g+="</span>")});e.insertAdjacentHTML("beforeend",'<span id="'+h+'">'+k+g+"</span>");b=this.getElement(b);c=this.getElement(c);b&&this.addCustomEvent(b,this.getConfig("event","file_remove"),this.removeFile.bind(this,h,this.createNewFile(a,a.name)));c&&this.addCustomEvent(c,this.getConfig("event","file_download"),this.downloadFile.bind(this,h,this.createNewFile(a,a.name)))};d.prototype.removeFile=
function(a,b,c){if(this.isIngRemove)return alert(this.getConfig("message","file_remove_is_ing")),!1;if(!confirm(this.getConfig("message","file_remove")))return!1;this.isIngRemove=!0;this.preventEvent(c);var e=this;c=new g;var d=b.name,f=e.getElement(a),h=e.makeParamFile(b),k=e.getConfig("url","file_remove");c.error(function(a){console.error("[SFM] fail remove file:",k,a);e.isIngRemove=!1;alert(e.getConfig("message","file_remove_error"))});c.then(function(a,c){e.callConfigEventHandler("file_remove_before",
{file:b,item:f})?a():c()}).then(function(a,b){null==k||""===e.trim(k)?a():e.ajax.post(k,h,function(b,c,e){a([b,c,e])},function(a){b();throw Error();})}).then(function(a,b,c){e.removeSelf(f);e.fileMap.remove(d);e.callConfigEventHandler("file_remove_after",c);e.isIngRemove=!1;e.insertDefaultHTML()})};d.prototype.downloadFile=function(a,b,c){this.isIngDownload=!0;this.preventEvent(c);var e=this;c=new g;var d=e.getElement(a),f=e.getConfig("url","file_download");c.error(function(a){console.error("[SFM] fail download file:",
f,a);alert(e.getConfig("message","file_download_error"))});c.then(function(a,c){e.callConfigEventHandler("file_download_before",{file:b,item:d})?a():c()}).then(function(a,b){if(null==f||""==e.trim(f))console.warn("[SFM] downloadURL is not define"),a();else try{location.href=f,a()}catch(l){throw b(),Error();}}).then(function(){e.callConfigEventHandler("file_download_after");e.isIngDownload=!1})};d.prototype.uploadFile=function(){if(this.isIngUpload)return alert(this.getConfig("message","file_upload_is_ing")),
!1;this.isIngUpload=!0;var a=this,b=new g,c=a.makeParamFiles(null,!0),e=a.getConfig("url","file_upload");b.error(function(b){console.error("[SFM] fail upload:",e,b);a.isIngUpload=!1;alert(a.getConfig("message","file_upload_error"))});b.then(function(b,c){a.callConfigEventHandler("file_upload_before")?b():c()}).then(function(b,d){null==e||""===a.trim(e)||0==a.fileMap.length?b():a.ajax.post(e,c,function(a,c,d){b([a,c,d])},function(a){d();throw Error();})}).then(function(b,c,d){a.callConfigEventHandler("file_upload_after",
d);a.isIngUpload=!1})};d.prototype.removeSelf=function(a){a.parentNode.removeChild(a)};d.prototype.makeParamFile=function(a){var b=new FormData;this.setParamFormData(b,a,!1);return b};d.prototype.makeParamFiles=function(a,b){a=a?a:new FormData;var c=this.fileMap.array(),d=0;b=b?b:!1;c.forEach(function(c){if(!b||b&&c.isNewFile)this.setParamFormData(a,c,!0,d,b),d++}.bind(this));return a};d.prototype.setParamFormData=function(a,b,c,d){var e=this.addSeparator(this.elementId.item,b.name);e=this.getElement(e).querySelector("["+
this.elementId.itemKeyArea+"]").childNodes;var g=this.getConfig("key"),h;for(h=0;h<e.length;h++){var k=e[h];f.call(g,k.id)&&a.append(this.getParamName(c,!1,d,k.id),"number"===g[k.id].toLowerCase()?Number(k.value):String(k.value))}a.append(this.getParamName(c,!0,d),b,b.name)};d.prototype.getParamName=function(a,b,c,d){c=this.getConfig("file","file_list_parameter_name")+"["+c+"].";var e=this.getConfig("file","file_parameter_name");return a?b?c+e:c+d:b?e:d};d.prototype.getFileToFormData=function(a){return this.makeParamFiles(a)};
...
this._$sfmEventListeners[a]);delete this._$sfmEventListeners[a]}.bind(this)),delete this._$sfmEventListeners)}.bind(this));return this};d.prototype.callConfigEventHandler=function(a,b){var c=this.option;var d=this.searchProp(this.config,["eventHandler",a]);d=Array.isArray(b)?d.apply(this.callee,b):d.call(this.callee,b);!1!==d&&f.call(c,"eventHandler")&&f.call(c.eventHandler,a)&&(c=c.eventHandler[a],d=Array.isArray(b)?c.apply(this.callee,b):c.call(this.callee,b));return!1===d?!1:!0};d.prototype.preventEvent=
...
new XMLHttpRequest:new ActiveXObject("Microsoft.XMLHTTP");e.onload=function(a){4==this.readyState&&200==this.status?d&&d(a.target.response,a,e):f&&f(e)};e.open(a,b,!0);e.setRequestHeader("encType","multipart/form-data");e.send(c)}};var k=function(a){if(!(this instanceof k))throw new SyntaxError('[SFMGrid] "new" constructor operator is required');this.requiredValidator(a);this.fileMap=new h;this.rowIdColumn=a.rowIdColumn;this.listParameterName=a.listParameterName;this.fileParameterName=a.fileParameterName||
"file";this.callee={};this.init();return this.callee};k.prototype.requiredValidator=function(a){if(null==a||!f.call(a,"rowIdColumn"))throw new SyntaxError('[SFMGrid] constructor property "rowIdColumn" is required');if(!f.call(a,"listParameterName"))throw new SyntaxError('[SFMGrid] constructor property "listParameterName" is required');f.call(a,"fileParameterName")||console.warn('[SFMGrid] constructor property "fileParameterName" is not define')};k.prototype.init=function(){this.makeCallee()};k.prototype.makeCallee=
function(){this.callee.get=this.get.bind(this);this.callee.set=this.set.bind(this);this.callee.remove=this.remove.bind(this);this.callee.clear=this.clear.bind(this);this.callee.getFormDataFromObject=this.getFormDataFromObject.bind(this);this.callee.getFormDataFromArray=this.getFormDataFromArray.bind(this)};k.prototype.get=function(a){return this.fileMap[a]};k.prototype.set=function(a,b){this.fileMap[a]=b};k.prototype.remove=function(a){delete this.fileMap[a]};k.prototype.clear=function(a){this.fileMap.clear()};
k.prototype.getFormDataFromObject=function(a){a=new FormData;for(var b in this.fileMap)this.rowIdColumn===b&&f.call(this.fileMap,this.fileMap[this.rowIdColumn])&&a.append(this.fileParameterName,this.fileMap[this.fileMap[this.rowIdColumn]]),a.append(b,this.fileMap[b]);return a};k.prototype.getFormDataFromArray=function(a){if(0==a.length)return null;var b=this,c=new FormData,d=this.copyObject(this.fileMap);a.forEach(function(a,e){for(var g in a)b.rowIdColumn===g&&f.call(d,a[b.rowIdColumn])&&(c.append(b.listParameterName+
"["+e+"]."+b.fileParameterName,d[a[b.rowIdColumn]]),delete d[a[b.rowIdColumn]]),c.append(b.listParameterName+"["+e+"]."+g,a[g])});return c};k.prototype.copyObject=function(a){var b={},c;for(c in a)b[c]=a[c];return b};return{SFM:d,SFMGrid:k}}(function(){var h=function(){this.tail=this.head=null;this.map=Object.create(null);this.length=0;this.hasProp=Object.prototype.hasOwnProperty};h.prototype.clear=function(){this.tail=this.head=null;this.map=Object.create(null);this.length=0};h.prototype.get=function(g){return(this.hasProp.call(this.map,
g)?this.map[g]:{}).value};h.prototype.size=function(){return this.map.length};h.prototype.getByIndex=function(g){return this.getItemByIndex(g).value};h.prototype.getIndexByKey=function(g){return this.getItemByKey(g).index};h.prototype.getItemByKey=function(g){var f=null;this.each(function(d,h){if(g===d.key)return f={index:h,key:d.key,value:d.value},!1});return f};h.prototype.getItemByIndex=function(g){var f=null;this.each(function(d,h){if(g==h)return f={index:h,key:d.key,value:d.value},!1});return f};
h.prototype.isContainsKey=function(g){return this.hasProp.call(this.map,g)?!0:!1};h.prototype.put=function(g,f){this.remove(g);var d={key:g,value:f,prev:this.tail,next:null};this.map[g]=d;0===this.length&&(this.head=d);this.tail&&(this.tail.next=d);this.tail=d;this.length+=1};h.prototype.remove=function(g){if(this.hasProp.call(this.map,g)){var f=this.map[g];this.head===f&&(f.next&&(f.next.prev=null),this.head=f.next);this.tail===f&&(f.prev&&(f.prev.next=null),this.tail=f.prev);f.next&&(f.next.prev=
f.prev);f.prev&&(f.prev.next=f.next);delete this.map[g];--this.length}};h.prototype.each=function(g){for(var f=0,d=this.length,h=this.head,a;f<d;){a={key:h.key,value:h.value};if(!1===g(a,f))break;h=h.next;f++}};h.prototype.eachRvs=function(g){for(var f=this.length,d=this.tail,h;0<f;){h={key:d.key,value:d.value};if(!1===g(h,f))break;d=d.prev;f--}};h.prototype.array=function(){var g=[];this.each(function(f,d){g.push(f.value)});return g};h.prototype.arrayRvs=function(){var g=[];this.reverseEach(function(f,
d){g.push(f.value)});return g};return h}(),function(){var h=function(){this.queue=[];this.isIng=!1;this.onReject=this.onError=null};h.prototype.run=function(g){if(0<this.queue.length&&!this.isIng){this.isIng=!0;try{this.queue.shift()(this.makeResolve(),this.makeReject(),g)}catch(f){this.onError?this.onError(f):console.error(f.message)}}};h.prototype.then=function(g){this.queue.push(g);this.run();return this};h.prototype.error=function(g){this.onError=g;return this};h.prototype.stop=function(){this.queue=
[];this.isIng=!1};h.prototype.reject=function(g){this.onReject=g;return this};h.prototype.makeResolve=function(){return function(g){this.isIng=!1;this.run(g)}.bind(this)};h.prototype.makeReject=function(){return function(g){this.stop();if(this.onReject)this.onReject(g)}.bind(this)};return h}()));